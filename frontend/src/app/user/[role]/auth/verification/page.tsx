...
import { AuthResponse, handleVerification } from "@/services/authApi";
...
const UserVerification = () => {
	...
	>({
		mutationFn: () => handleVerification(user.role, user.email),
		onSuccess: (data) => {
			if (data) {
	...
	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		mutate({ role: user.role, email: user.email });
		setOpen(true);
	};
	...
	return (
		<>
			...
			<form
				className='flex flex-col gap-3 w-full lg:w-2/3  p-4 rounded-md border-slate-300'
				...
				<div className='flex justify-center items-center'>
					<button
						type='submit'
						name='submit'
						className='flex items-center justify-center h-12 min-w-24 capitalize p-4 rounded-md bg-emerald-400 text-slate-200 gap-2'>
						{isPending && (
							<span role='status'>
								<svg
									aria-hidden='true'
